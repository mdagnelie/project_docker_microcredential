...
On my terminal, `docker build --tag ml-train:v0.1 -f Dockerfile.train .`
...
then to create a container actually running the python script "train.py": `docker run -u 501:20 -v ./models:/app/models ml-train:v0.1`
...
On my terminal, `docker build --tag ml-serve:v0.1 -f Dockerfile.infer .`
Once the image is created, run a container from it with `docker run -u 501:20 -p 8080:8080 -v ./models:/app/models ml-serve:v0.1`
